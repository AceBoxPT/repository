...
def menu_principal():
      addDir(traducao(30012),MainURL,3,os.path.join(art,'1.png'),1)
...
def epocasanteriores(url):
      ...
      for endereco,titulo in anteriores:
            addDir(titulo,MainURL + endereco,8,os.path.join(art,'next.png'),len(anteriores))
      if "confluence" in xbmc.getSkinDir(): xbmc.executebuiltin("Container.SetViewMode(51)")

def request(url,special=False):
      link=abrir_url(url).replace('&nbsp;','')
      if special:
            listagolos=re.compile('<div id=".+?"><a href="(.+?)"><img.+?src="images/(.+?)\..+?" />\s*(.+?)</a></div>').findall(link)
            for endereco,thumb,multiple in listagolos:
                  ...
                  addDir('[COLOR blue]%s[/COLOR][COLOR darkblue](%s)[/COLOR][COLOR blue] - [/COLOR][COLOR white]%s[/COLOR]' % (data,hora,rest),MainURL + endereco,1,os.path.join(art,'%s.png' % (thumb)),len(listagolos),pasta=False)
      else:
            ...
            for endereco,thumb,data,hora,equipa1,resultado1,resultado2,equipa2 in listagolos:
                  addDir('[COLOR blue]%s[/COLOR] [COLOR darkblue]%s[/COLOR][COLOR blue] - [/COLOR][COLOR white]%s[/COLOR] [COLOR yellow]%s - %s[/COLOR] [COLOR white]%s[/COLOR]' % (data,hora,equipa1,resultado1,resultado2,equipa2),MainURL + endereco,1,os.path.join(art,'%s.png' % (thumb)),len(listagolos),pasta=False)
      ...
      if re.search('page-start', link):
            try:
                  ...
                  addDir('[COLOR blue][B]%s %s >[/COLOR][/B]' % (traducao(30003),pagina),MainURL + enderecopagina,mode,os.path.join(art,'next.png'),1)
            except: pass
...
def addDir(name,url,mode,iconimage,total,pasta=True):
      u=sys.argv[0]+"?url="+urllib.quote_plus(url)+"&mode="+str(mode)+"&name="+(name)+"&thumb="+urllib.quote_plus(iconimage)
...
def pesquisa():
      ...
      if pesquisa:
            ...
            for endereco,data,hora,equipa1,resultado1,resultado2,equipa2 in jogos:
                  addDir('[COLOR blue]%s[/COLOR] [COLOR darkblue]%s[/COLOR][COLOR blue] - [/COLOR][COLOR white]%s[/COLOR] [COLOR yellow]%s - %s[/COLOR] [COLOR white]%s[/COLOR]' % (data,hora,equipa1,resultado1,resultado2,equipa2),MainURL + endereco,1,os.path.join(art,'next.png'),len(jogos),pasta=False)
            if "confluence" in xbmc.getSkinDir(): xbmc.executebuiltin("Container.SetViewMode(51)")
      else: sys.exit(0)
